...
TPL="$(cd "$(dirname "$0")" && pwd)/remotion-template"
R="$W/remotion"
[ -d "$TPL/src" ] || { echo "❌ شاشة Remotion الاختيارية تحتاج مجلد scripts/remotion-template الكامل؛ المحرك الأساسي يعمل من دونه."; exit 15; }
...
case "$CMD" in
  ...
  render)
    sync_all; OUT="${ARG:-$W/reel-final.mp4}"
